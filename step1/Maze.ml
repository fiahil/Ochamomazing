...
let create width high =
  ...
  let rec create_map map width =
    function
      ...
      | n	->
	begin
          Array.set map n (create_line (cm_aux width n) (n * width) (width - 1));
          create_map map width (n - 1)
	end
  and
      cm_aux w n = Array.make w (Case.create (n * w))
  in
...
let colorize maze width high =
  ...
  let change_case_by_color color new_color =
    let rec change_columns x =
      function
        | -1	-> ()
        | y	->
          if (Case.color (get_case_at_pos maze (x, y)) == color) then
            begin
              Array.set maze.(x) y (cc_aux x y new_color);
              change_columns x (y - 1)
            end
          else
            change_columns x (y - 1)
    and
	cc_aux x y color = (Case.set_color (get_case_at_pos maze (x, y)) color)
    in
  ...
  let change_case case dir =
	...
	Case.color (get_case_at_pos maze (add_tuple dir case))) then
      begin
	  ...
	  (change_wall
        ...
        Array.set maze.(get_first case)
	  (get_second case)
	  (change_wall (get_case_at_pos maze case) dir);
        change_case_by_color (Case.color (get_case_at_pos maze (add_tuple dir case)))
	  (Case.color (get_case_at_pos maze case)); (* prendre la color min *)
        1
      end
    else
      0
